...
<section class=\"ias-panel\" aria-label=\"Recommendation actions\"><h2>Actions</h2><button type=\"button\" aria-disabled=\"true\" disabled>Approve</button> <button type=\"button\" aria-disabled=\"true\" disabled>Reject</button> <button type=\"button\" aria-disabled=\"true\" disabled>Export CSV</button><p class=\"ias-muted\">Actions are shown but disabled until recommendation transition endpoints are available in the next API batch.</p></section>
"""
