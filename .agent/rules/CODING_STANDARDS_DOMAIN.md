# Inventory Allocation Simulator — Coding Standards: Domain & Production
...
2. Tests run against local services (local PostgreSQL, local Redis, local DuckDB)
...
Detailed security rules live in `CODING_STANDARDS_SECURITY.md` and are core-loaded via `_index.md`. Follow them for secrets, input validation, auth, SQL/data safety, and multi-tenant config-driven surfaces.
...
1. **All tests pass** — `python -m pytest` / `julia --project -e 'using Pkg; Pkg.test()'` / equivalent shows 0 failures
