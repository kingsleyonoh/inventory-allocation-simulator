# Inventory Allocation Simulator — Coding Standards: E2E Testing (Real Endpoints)
...
1. Start the actual server: `julia --project src/Main.jl` (NOT a test-mode in-process server)
...
| Run command | `julia --project -e 'using Pkg; Pkg.test()'` | `npx playwright test` |
...
- Document `npx playwright test` as the E2E command (or add equivalent project script if Node tooling is introduced)
